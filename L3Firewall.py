...
class Firewall (EventMixin):
	...
	def __init__ (self,l2config,l3config):
		...
		self.fwconfig = list()
                ...
                # Port table will store our known IP - MAC combos
                self.portTable = {}
		'''
		Read the CSV file
		'''
  ...
  # ZACH -- Turn this off!!!!
	...
	def replyToIP(self, packet, match, event, fwconfig):
		...
		nwproto = str(match.nw_proto)
                ...
                # ZACH -- portTable Logic
                if match.dl_src not in self.portTable:
                    print("Adding true MAC id: " + str(match.dl_src) + " For IP: " + str(match.nw_src))
                    self.portTable[match.dl_src] = match.nw_src
                else:
                    if self.portTable[match.dl_src] !=  match.nw_src:
                        print("Src Mac and Src IP don't match records, dropping packet and adding rule to L2 firewall")
                        message = of.ofp_flow_mod()
                        match_flow = of.ofp_match()
                        match_flow.dl_src = match.dl_src
                        match_flow.dl_dst = match.dl_dst
                        message.priority = 65535
                        message.match = match_flow
                        message.hard_timeout = 0
                        message.idle_timeout = 60 
